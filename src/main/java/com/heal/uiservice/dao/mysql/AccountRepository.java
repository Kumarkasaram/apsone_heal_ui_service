...
public interface AccountRepository<T extends Account, E extends Serializable> extends JpaRepository<T, E> {
    ...
    @Query("select new com.api.appsoneapi.beans.AccountBean (a.id ,0,mt.timeoffset,mt.timeoffset, a.name ,a.identifier,mt.timeZoneId, a.updatedTime, a.userDetailsId ,mt.offsetName, mt.abbreviation,a.userDetailsId) FROM Account a join TagMapping as tm on tm.objectId = a.id join TagDetails as td on tm.tagId = td.id join MstTimezone as mt on mt.id = tm.tagKey where tm.objectRefTable =:accountTableName and td.name = :timezoneKey and a.status = 1")
    List<AccountBean> getAccountDetails(@PathParam("timezoneKey") String timezoneKey, @PathParam("accountTableName") String accountTableName);**/
